...
const requiredPatterns = [
  ...
  ['Secret Manager API verification', /secretmanager\.googleapis\.com/],
  ...
  ['customer secret metadata lookup', /gcloud secrets list/],
  ['Cloud Run secret binding', /--set-secrets/],
  ['stale customer secret cleanup', /--remove-secrets/],
  ['no-traffic candidate deployment', /--no-traffic/],
  ['readiness precedes promotion', /Wait for ready Cloud Run revision[\s\S]*gcloud run services update-traffic/],
  ['candidate revision tag', /CANDIDATE_TAG/],
  ['Cloud Run revision output', /revision-name/],
  ['candidate revision URL output', /candidate-url/],
  ...
  ['deployment result URL report', /serviceUrl/],
...
for (const templatePath of callerTemplatePaths) {
  ...
  const callerRequirements = [
    ...
    ['branch to environment mapping', /github\.ref_name\s*==\s*'main'\s*&&\s*'prod'.*github\.ref_name\s*==\s*'uat'\s*&&\s*'uat'.*'preview'/s],
