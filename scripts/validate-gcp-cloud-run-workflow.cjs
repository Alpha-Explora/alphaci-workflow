...
const requiredPatterns = [
  ...
  ['prod branch maps to prod', /\$\{BRANCH\}"\s*==\s*"prod".*\$\{ENVIRONMENT\}"\s*==\s*"prod"/s],
...
for (const templatePath of callerTemplatePaths) {
  ...
  const callerRequirements = [
    ...
    ['branch to environment mapping', /github\.ref_name\s*==\s*'prod'\s*&&\s*'prod'.*github\.ref_name\s*==\s*'uat'\s*&&\s*'uat'.*'dev'/s],
